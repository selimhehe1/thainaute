...
const lire = (dossier, id) =>
  JSON.parse(
    readFileSync(
      join(RACINE, "packages", "content", "data", dossier, `${id}.v1.json`),
      "utf8",
    ),
  );
...
const parGraphieSource = new Map(
  leconSource.items.map((i) => [i.thaiRaw, i.id]),
);
const parGraphieCible = new Map(leconCible.items.map((i) => [i.thaiRaw, i.id]));
const graphieDeSource = new Map(
  [...parGraphieSource].map(([g, id]) => [id, g]),
);
...
for (const entree of manifesteSource.entries) {
  ...
  const idCible =
    graphie === undefined ? undefined : parGraphieCible.get(graphie);
...
if (entrees.length === 0) {
  console.error(
    "aucune entrée réutilisable : les deux leçons ne partagent aucune graphie",
  );
  process.exit(1);
}
...
for (const [champ, valeur] of Object.entries(manifeste)) {
  if (valeur === undefined) {
    console.error(
      `champ ${champ} absent de la leçon ${cible} : manifeste non écrit`,
    );
    process.exit(1);
  }
}
...
console.log(
  `${cible} : ${entrees.length} entrées réemployées depuis ${source}`,
);
for (const e of entrees) {
  console.log(
    `   ${e.synthesis?.sourceText ?? "?"}  ->  ${e.distributionPaths[0]}`,
  );
}
if (ignorees.length > 0) {
  console.log(
    `   ${ignorees.length} entrées ignorées (graphie absente de ${cible}) : ${ignorees.join(" ")}`,
  );
}
