// Balayage par SOUS-CHAÎNE de la colonne THA de VOLUBILIS.
// Même exemplaire, même méthode d'extraction que volubilis-lookup-full.mjs.
// Motif : la comparaison EXACTE du script versionné ne peut pas atteindre les
// cellules de la forme « ล่ะ (...ล่ะ) », piège déjà consigné par u11-l11a.
...
const workDir = mkdtempSync(join(tmpdir(), "vol-sub-"));
try {
  execFileSync(
    "powershell",
    [
  ...
  for (const word of words) {
    ...
    for (const [row, values] of cells) {
      ...
      if (tha.slice(2).normalize("NFC").includes(target))
        hits.push({ row, values });
    }
    hits.sort((a, b) => a.row - b.row);
    console.log(
      `### sous-chaîne « ${target} » en colonne E : ${hits.length} ligne(s)`,
    );
