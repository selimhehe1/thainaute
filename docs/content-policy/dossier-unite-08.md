# Dossier de preuve, unité 08

Document GÉNÉRÉ par `scripts/content/dossier-de-preuve.mjs`. Il est
recalculé depuis les paquets compilés, les manifestes audio et les dossiers
de vérification réellement présents dans le dépôt. Le régénérer après toute
modification de contenu.

Il ne vaut pas approbation. Il rassemble ce qui existe et nomme ce qui
manque, pour qu'une signature soit un acte informé.

## État de la chaîne

- Leçons compilées : 5
- Audits déclarés : 35 passés sur 35
- Findings bloquants encore ouverts : 0
- Nature des auteurs déclarés : ai
- Nature des auditeurs déclarés : ai

**Aucun auteur ni auditeur humain n'est encore enregistré.** Les portes
`HUMAN_AUTHOR_MISSING` et `HUMAN_AUDITOR_MISSING` restent donc fermées, et
c'est exactement ce que la signature du fondateur vient lever.

**Revue par un locuteur natif : en attente.** Elle n'a pas eu lieu, et
aucune signature ne peut en tenir lieu.

## u08-l8a : Le ng du début et les voyelles qui glissent

- Objectif : à la fin de la leçon, devant un mot entendu, l’apprenant dit si le son `ng` tombe au début du mot, à la fin, ou aux deux, sur 8 mots sur 10 ; il reconnaît à l’écoute laquelle des trois voyelles glissées `ia`, `uea` et `oua` a été prononcée, sur 7 tirages sur 9 ; devant un mot écrit en syllabe vivante, il annonce le ton en croisant la marque, y compris les deux marques nouvelles, et la classe de la
- Statut : `draft`, visibilité `internal`
- Items : 8
- Exercices : 8 (recall 8)
- Fichiers audio : 0

Dossier de vérification : `verification-8a.md`.

| Graphie | Transcription | Français                                                                                 | Sources                                 |
| ------- | ------------- | ---------------------------------------------------------------------------------------- | --------------------------------------- |
| เงิน    | ngoen         | l’argent (la monnaie)                                                                    | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN |
| ง่าย    | ngâai         | facile, simple                                                                           | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN |
| เสีย    | sǐa           | être hors d’usage, en panne, ne plus marcher ; s’abîmer, et pour un aliment avoir tourné | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN |
| เสื้อ   | sûea          | le haut, la chemise (le vêtement du buste)                                               | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN |
| ตั๋ว    | tǒua          | le billet, le ticket                                                                     | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN |
| เก๊     | kée           | faux, contrefait                                                                         | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN |
| เปลี่ยน | plìan         | changer, échanger                                                                        | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN |
| กระเป๋า | krà·pǎo       | le sac                                                                                   | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN |

## u08-l8b : Chercher, essayer, choisir

- Objectif : à la fin de la leçon, devant une situation décrite en français, l’apprenant relie le bon verbe parmi หา, อยาก, อยากได้, ต้องการ, ลองดู et เอา, sur 5 appariements justes du premier coup sur 6, les deux verbes qui prennent une CHOSE y étant acceptés indifféremment l’un pour l’autre ; il construit sans erreur d’ordre et sans bloc en trop les quatre phrases de l’exercice 3, dont deux opposent directem
- Statut : `draft`, visibilité `internal`
- Items : 11
- Exercices : 16 (audio_choice 6, word_order 4, reading 6)
- Fichiers audio : 0

Dossier de vérification : `verification-8b.md`.

| Graphie   | Transcription | Français                                              | Sources                                                |
| --------- | ------------- | ----------------------------------------------------- | ------------------------------------------------------ |
| หา        | hǎa           | chercher                                              | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| อยาก      | yàak          | avoir envie de, vouloir (suivi d’un verbe)            | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| อยากได้   | yàak·dâai     | vouloir, aimerais avoir (suivi d’une chose)           | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| ต้องการ   | tâwng·kaan    | vouloir, avoir besoin de                              | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| ลอง       | lawwng        | essayer                                               | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| ลองดู     | lawwng douu   | essayer, pour voir                                    | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| เอา       | ao            | prendre ; vouloir                                     | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| เอาอันนี้ | ao an·níi     | je prends celui-ci                                    | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| ผม        | phǒm          | je, moi (locuteur homme) ; par ailleurs : les cheveux | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| ดิฉัน     | dì·chǎn       | je, moi (locutrice femme, échange poli)               | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| ห้า       | hâa           | cinq (le nombre 5)                                    | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |

## u08-l8c : Trop cher, trop grand, trop petit

- Objectif : à la fin de la leçon, l’apprenant identifie à l’écoute lequel de trois blocs a été prononcé, alors que deux d’entre eux ne diffèrent que par la présence de เกินไป, sur 6 tirages sur 8 ; il associe les six blocs du jour à leur sens français, sur 5 paires sur 6 ; il construit par blocs quatre phrases sur cinq en plaçant เกินไป APRÈS le mot qu’il modifie, dont deux phrases où เกินไป ne doit pas appar
- Statut : `draft`, visibilité `internal`
- Items : 11
- Exercices : 13 (audio_choice 8, word_order 5)
- Fichiers audio : 0

Dossier de vérification : `verification-8c.md`.

| Graphie             | Transcription                  | Français                  | Sources                                                |
| ------------------- | ------------------------------ | ------------------------- | ------------------------------------------------------ |
| ถูก                 | thòuuk                         | bon marché, d’un prix bas | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| ใหญ่                | yài                            | grand, de grande taille   | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| เล็ก                | lék                            | petit, de petite taille   | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| เกินไป              | koeen·pai                      | trop, excessivement       | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| แพงเกินไป           | phaeeng koeen·pai              | trop cher                 | RID-2554, VOLUBILIS-26-2                               |
| ใหญ่เกินไป          | yài koeen·pai                  | trop grand                | RID-2554, VOLUBILIS-26-2                               |
| เล็กเกินไป          | lék koeen·pai                  | trop petit                | RID-2554, VOLUBILIS-26-2                               |
| อันนี้แพงเกินไปครับ | an·níi phaeeng koeen·pai khráp | celui-ci est trop cher    | RID-2554, VOLUBILIS-26-2                               |
| แพง                 | phaeeng                        | cher (d’un prix élevé)    | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| อันนี้              | an·níi                         | ceci, celui-ci            | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| อันนี้เท่าไรครับ    | an·níi thâo·rai khráp          | Ça coûte combien ?        | RID-2554, VOLUBILIS-26-2                               |

## u08-l8d : Quand ça ne va pas

- Objectif : à la fin de la leçon, l’apprenant identifie à l’écoute laquelle des trois formules du jour a été prononcée, มีปัญหา, อันนี้ไม่ใช่ ou ขอเปลี่ยนหน่อย, 6 fois sur 8, alors que la particule finale est présente dans les huit énoncés et ne distingue aucune option ; il associe six situations françaises à la formule thaïe qui convient, 5 paires sur 6 ; il assemble dans l’ordre les cinq demandes de l’exerc
- Statut : `draft`, visibilité `internal`
- Items : 7
- Exercices : 5 (word_order 5)
- Fichiers audio : 0

Dossier de vérification : `verification-8d.md`.

| Graphie | Transcription | Français                                                                    | Sources                                                |
| ------- | ------------- | --------------------------------------------------------------------------- | ------------------------------------------------------ |
| ไม่ใช่  | mâi·châi      | ce n’est pas ça ; non                                                       | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| ปัญหา   | pan·hǎa       | le problème                                                                 | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| เปลี่ยน | plìan         | changer, remplacer une chose par une autre                                  | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| เสีย    | sǐa           | être hors d’usage, en panne, abîmé ; avoir tourné, pour un aliment          | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| ขอ      | khǎww         | demander ; solliciter (et, en tête de phrase, « je voudrais », « puis-je ») | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| อันนี้  | an·níi        | ceci, celui-ci                                                              | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
| มี      | mii           | avoir ; posséder                                                            | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |

## u08-l8e : Au magasin

- Objectif : à la fin de la leçon, l’apprenant suit sans texte affiché un échange de 8 répliques entre un homme et une femme dans un magasin de vêtements, et répond correctement à 5 questions sur 6 ; il reconstruit la réclamation ขอ เปลี่ยน หน่อย ครับ et la demande de prix ตัวนี้ เท่าไร ครับ dans cet ordre, avec la particule qui correspond au locuteur indiqué, soit 2 tirages sur 2 ; il écrit en transcription 3
- Statut : `draft`, visibilité `internal`
- Items : 12
- Exercices : 3 (word_order 2, association 1)
- Fichiers audio : 0

Dossier de vérification : `verification-8e.md`.

| Graphie          | Transcription           | Français                                                                    | Sources                                                |
| ---------------- | ----------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------ |
| ตัวนี้           | toua·níi                | celui-ci (en parlant d’un objet que l’on compte avec ตัว)                   | RID-2554, WIKTIONARY-EN                                |
| หา               | hǎa                     | chercher                                                                    | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| เสื้อ            | sûea                    | le haut, la chemise (le vêtement du buste)                                  | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| ตัว              | toua                    | classificateur des animaux, et des vêtements à manches ou à jambes          | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| เงิน             | ngoen                   | l’argent (la monnaie)                                                       | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| เกินไป           | koeen·pai               | trop, excessivement                                                         | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| แพงเกินไป        | phaeeng koeen·pai       | trop cher                                                                   | RID-2554, VOLUBILIS-26-2                               |
| ใหญ่เกินไป       | yài koeen·pai           | trop grand                                                                  | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| ปัญหา            | pan·hǎa                 | le problème                                                                 | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| เปลี่ยน          | plìan                   | changer, échanger                                                           | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN                |
| ตัวนี้เท่าไรครับ | toua·níi thâo·rai khráp | Celui-ci, c’est combien ?                                                   | RID-2554                                               |
| ขอ               | khǎww                   | demander ; solliciter (et, en tête de phrase, « je voudrais », « puis-je ») | RID-2554, VOLUBILIS-26-2, WIKTIONARY-EN, WIKTIONARY-TH |
